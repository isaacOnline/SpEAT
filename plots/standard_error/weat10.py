...
def perform_tests(model_name, target_voices, attribute_dataset, dimension, timestamp, npermutations = 10000, nlayers=13):
    ...
    target_embeddings = np.stack(
        [np.load(os.path.join('embeddings', 'speech_accent_archive', dataset_name, model_name, f'layer_{i}',
                              f'mean.npy')) for i in
         range(nlayers)], axis=2).sum(axis=2)

    speaker_info = load_dataset_info(os.path.join('speech_accent_archive',dataset_name))

    samples_per_gender_max = speaker_info.groupby(['sex','age_rank'])['id'].count().min()
    ...
    attribute_embeddings = np.stack(
        [np.load(os.path.join('embeddings', valence_model_dataset, model_name, f'layer_{i}', f'mean.npy')) for i
    ...
    model_log_path = os.path.join('plots', 'standard_error', 'young_old_audio_iat.log')
    ...
    logger.info(f'Using model {model_name}, Emotion Dataset {valence_model_dataset}, '
                f'Dimension {dimension}, Timestamp {timestamp}, Target Dataset {target_dataset_full_name}')
    logger.info(f'Using seed: {seed}')

    with tqdm(total = npermutations * len(range(1, samples_per_gender_max+1))) as pbar:

        for sample_per_gender in range(1, samples_per_gender_max+1):
            n_target_embd = sample_per_gender * 2

            if not check_if_se_already_run('Young vs. Old Accents',
                                            'Nosek et al.',
                ...
                for _ in range(npermutations):
                    sampled_info = speaker_info.groupby(['sex','age_rank']).sample(sample_per_gender, replace=True)

                    high_age = sampled_info[sampled_info['age_rank'] == 'high_age']
                    low_age = sampled_info[sampled_info['age_rank'] == 'low_age']
                    high_age_embd = target_embeddings[high_age['index'].tolist()]
                    low_age_embd = target_embeddings[low_age['index'].tolist()]
                ...
                logger.info(f'For sample size {sample_per_gender * 2}, stdev of effect sizes for resampled '
                            f'embeddings is {stdev}')

                write_eat_se_result('Young vs. Old Accents',
                                 'Nosek et al.',
                                 names, n_target_embd, n_attribute_embd, target_dataset_full_name, attribute_dataset,
                                 attribute_dataset, model_name,
...
if __name__ =='__main__':
    ...
    for valence_model_dataset, _, dimension, model_name, nlayers, timestamp in to_test:
        ...
        for voice_type in target_voices:
            perform_tests(model_name, voice_type, valence_model_dataset, dimension, timestamp, nlayers=nlayers)
