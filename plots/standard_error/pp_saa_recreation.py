...
def perform_tests(model_name, attribute_dataset, dimension, timestamp, nlayers=13, npermutations=10000, embedding_aggregation_method=('mean', 'mean')):
    ...
    target_embeddings = load_embeddings(os.path.join('speech_accent_archive', 'us_korean'), model_name, nlayers, embedding_aggregation_method)
    speaker_info = load_dataset_info(os.path.join('speech_accent_archive', 'us_korean'))

    target_dataset = ('SAA (U.S. vs. Korean Matches)')
    ...
    for n_target_embd in range(2, len(speaker_info) // 2 + 1, 2):
        if not check_if_se_already_run('American vs. Korean English Accents',
            ...
            for _ in range(npermutations):
                ...
                american = sampled_info[sampled_info['language'] == 'english']
                foreign = sampled_info[sampled_info['language'] == 'korean']
