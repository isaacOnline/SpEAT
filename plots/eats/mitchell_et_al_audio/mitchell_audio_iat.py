...
def perform_tests(model_name, attribute_dataset, dimension, timestamp, npermutations=1000000, nlayers=13,
    ...
    # Test 1
    target_embeddings = load_embeddings(os.path.join('audio_iats', 'mitchell_et_al'), model_name, nlayers, embedding_aggregation_method)


    speaker_info = load_dataset_info(os.path.join('audio_iats', 'mitchell_et_al'))
    human = speaker_info[speaker_info['human_or_synthesized'] == 'human']
    synthesized = speaker_info[speaker_info['human_or_synthesized'] == 'synthesized']
    ...
    if not check_if_already_run('Human vs. Synthesized Speech',
        ...
        male = speaker_info[speaker_info['male_or_female'] == 'male']
        female = speaker_info[speaker_info['male_or_female'] == 'female']
