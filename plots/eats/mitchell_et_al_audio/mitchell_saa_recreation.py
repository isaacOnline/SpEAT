...
def perform_tests(model_name, attribute_dataset, dimension, timestamp, npermutations=1000000, nlayers=13,
    ...
    human = speaker_info[speaker_info['language'] == 'english']
    synthesized = speaker_info[speaker_info['language'] != 'english']
    ...
    n_target_embd = len(human_embeddings)
    target_dataset = 'Human Vs. Synthetic (SAA)'
    if not check_if_already_run('Human vs. Synthesized Speech',
                                target_dataset,
        ...
        write_eat_result('Human vs. Synthesized Speech',
                        target_dataset,
                         names, n_target_embd, n_attribute_embd, target_dataset, attribute_dataset,
                         attribute_dataset, model_name,
    ...
    target_embeddings = load_embeddings(os.path.join('speech_accent_archive', 'male_female'), model_name, nlayers, embedding_aggregation_method)
    speaker_info = load_dataset_info(os.path.join('speech_accent_archive', 'male_female'))


    male = speaker_info[speaker_info['sex'] == 'male']
    female = speaker_info[speaker_info['sex'] == 'female']
    ...
    names = ['Female','Male', f'Pleasant Audio Clips ({valence_model_dataset})',
         f'Unpleasant Audio Clips ({valence_model_dataset})']
    n_target_embd = len(male_embeddings)
    target_dataset = 'Female vs. Male (SAA)'
    if not check_if_already_run('Female vs. Male Speech',
                                target_dataset,
                                names, n_target_embd, n_attribute_embd, target_dataset, attribute_dataset,
                                attribute_dataset, model_name,
                                embedding_aggregation_method=embedding_aggregation_method):
        ...
        write_eat_result('Female vs. Male Speech',
                        target_dataset,
                         names, n_target_embd, n_attribute_embd, target_dataset, attribute_dataset,
                         attribute_dataset, model_name,
                         npermutations=npermutations,speat_d=test.effect_size(),speat_p=p,
                         iat_d='0.02 for males, 0.42 for females',
                         embedding_aggregation_method=embedding_aggregation_method)
    ...
    for valence_model_dataset, _, dimension, model_name, nlayers, timestamp in to_test:
        for layer_agg in ['mean',
            ...
            for seq_agg in [
                ...
                if (layer_agg, seq_agg) == ('q1', 'mean') and model_name == 'hubert_xtralarge_ll60k':
                    print('here')
