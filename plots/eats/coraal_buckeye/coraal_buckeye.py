...
def perform_tests(model_name, attribute_dataset, dimension, timestamp, npermutations=1000000, nlayers=13,
                  embedding_aggregation_method='mean'):

    seed = 24412168


    clip_info = load_dataset_info('coraal_buckeye_joined')
    target_embeddings = load_embeddings('coraal_buckeye_joined', model_name, nlayers, embedding_aggregation_method)

    aa_embeddings = target_embeddings[clip_info[clip_info['speaker_race'] == 'african_american'].index]
    ea_embeddings = target_embeddings[clip_info[clip_info['speaker_race'] == 'caucasian'].index]
    ...
    n_target_embd = len(aa_embeddings)
    ...
    if not check_if_already_run(f'({names[0]} vs. {names[1]}) vs. ({names[2]} vs. {names[3]})',
        ...
        write_eat_result(f'({names[0]} vs. {names[1]}) vs. ({names[2]} vs. {names[3]})',
                             ...
                             iat_d=0.73, embedding_aggregation_method=embedding_aggregation_method)
