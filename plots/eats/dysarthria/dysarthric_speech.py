...
def perform_tests(model_name, attribute_dataset, dimension, timestamp, npermutations=1000000, nlayers= 13, embedding_aggregation_method=('mean', 'mean')):
    ...
    target_embeddings = load_embeddings('UASpeech', model_name, nlayers, embedding_aggregation_method)

    dysarthric_embeddings = target_embeddings[clip_info[clip_info['type'] == 'dysarthric'].index]
    non_dysarthric_embeddings = target_embeddings[clip_info[clip_info['type'] == 'non_dysarthric'].index]
